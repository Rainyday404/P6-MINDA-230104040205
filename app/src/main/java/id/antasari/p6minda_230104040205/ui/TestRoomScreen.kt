...
// Update import data sesuai package baru
...
fun TestRoomScreen() {
    ...
    Column(
        ...
        // Tombol untuk Load Data Terakhir
        Button(
            onClick = {
                scope.launch(Dispatchers.IO) {
        ...
        // Menampilkan ID Terakhir yang di-insert
        Text(
            text = "LastInsertedId: ${lastInsertedId ?: "-"}"
        )

        // Menampilkan Detail Entry yang di-load
        Text(
            text = buildString {
                append("FetchedEntry:\n")
                val e = fetchedEntry
